import Scenery from './Scenery.jsx';
import { useAuth } from '../context/AuthContext.jsx';
import { useT } from '../i18n/index.jsx';
import LanguagePicker from './LanguagePicker.jsx';

const MODULES = [
  { key: 'nav.notes', hint: 'nav.notesHint', icon: '📝' },
  { key: 'nav.sheets', hint: 'nav.sheetsHint', icon: '📊' },
  { key: 'nav.slides', hint: 'nav.slidesHint', icon: '🖼️' },
  { key: 'nav.ai', hint: 'nav.aiHint', icon: '✨' },
  { key: 'nav.languages', hint: 'nav.languagesHint', icon: '🗣️' },
];

function GoogleMark() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true" focusable="false">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.91c1.7-1.57 2.69-3.88 2.69-6.62z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.91-2.26c-.81.54-1.84.86-3.05.86-2.34 0-4.32-1.58-5.03-3.7H.96v2.34A9 9 0 0 0 9 18z"
      />
      <path
        fill="#FBBC05"
        d="M3.97 10.72a5.4 5.4 0 0 1 0-3.44V4.94H.96a9 9 0 0 0 0 8.12l3.01-2.34z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.59C13.46.89 11.43 0 9 0A9 9 0 0 0 .96 4.94l3.01 2.34C4.68 5.16 6.66 3.58 9 3.58z"
      />
    </svg>
  );
}

export default function SignInScreen() {
  const { t } = useT();
  const { signIn, busy, error, isFirebaseConfigured } = useAuth();

  return (
    <div className="signin">
      <Scenery slot="signIn" className="signin-scene" overlay>
        <div className="signin-panel">
          <div className="signin-brand">
            <span className="signin-logo" aria-hidden="true">
              🎓
            </span>
            <div>
              <strong>{t('common.appName')}</strong>
              <small>{t('common.tagline')}</small>
            </div>
          </div>

          <h1>{t('auth.heroTitle')}</h1>
          <p className="signin-body">{t('auth.heroBody')}</p>

          <ul className="signin-modules">
            {MODULES.map((module) => (
              <li key={module.key}>
                <span aria-hidden="true">{module.icon}</span>
                <span>
                  <strong>{t(module.key)}</strong>
                  <small>{t(module.hint)}</small>
                </span>
              </li>
            ))}
          </ul>

          {isFirebaseConfigured ? (
            <>
              <button type="button" className="google-button" onClick={signIn} disabled={busy}>
                <GoogleMark />
                {busy ? t('auth.signingIn') : t('auth.signInWithGoogle')}
              </button>
              {error ? <p className="signin-error">{t('auth.signInError')}</p> : null}
              <p className="signin-privacy">{t('auth.privacy')}</p>
            </>
          ) : (
            <div className="signin-offline">
              <strong>{t('auth.offlineTitle')}</strong>
              <p>{t('auth.offlineBody')}</p>
            </div>
          )}

          <div className="signin-language">
            <LanguagePicker compact />
          </div>
        </div>
      </Scenery>
    </div>
  );
}
